# -*- coding: utf-8 -*-
"""
Created on May 11 10:07:26 2020
...
def main():
    ...
    convoluteTrainingData = True # Set to True if convolution is required
    ...
    if convoluteTrainingData:
        training_data, training_labels = getConvolutedData(n_conv_laps, alphabet)
    else:
...
def getConvolutedData(n_conv_laps, alphabet):
    ...
    for letter in alphabet:  # len(raw_training_data)
        conv_data = []
        conv_data.append(convoluteInputData(net_input[letter], n_conv_laps, vert_filt).flatten())
        conv_data.append(convoluteInputData(net_input[letter], n_conv_laps, hor_filt).flatten())
        conv_data.append(convoluteInputData(net_input[letter], n_conv_laps, diag_tl_br).flatten())
        conv_data.append(convoluteInputData(net_input[letter], n_conv_laps, diag_bl_tr).flatten())
        
        flattened_training_data.append(conv_data)
        
        label_as_array = [0.0] * len(net_input.keys)
        label_as_array[alphabet.index(letter)] = 1.0
    ...
    """
    return flattened_training_data, training_label_arrays #validating_label_arrays
...
def convoluteInputData(data, rem_laps, conv_filter):
    if len(data) < 5:   # Smallest allowed matrix size for one Conv revolution is 5x5.
        return data
    if rem_laps == 0:
        # print('Returned conv data of shape: ' + str(data.shape))
        return data
    # Filtrera
    ...
    #print('Conv filt time: ' + str(timer_conv_filt))    
    #print('Normalize time: ' + str(timer_normalize)) 
    #print('Pool time: ' + str(timer_pool))
